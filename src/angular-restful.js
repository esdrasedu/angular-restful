...
function RouteRestfulProvider() {

    this.$get = ['$http', '$parse', function($http, $parse) {
...
function RestfulProvider(){
    ...
    this.actions = function( actions ){
        providerActions = angular.extend({}, providerActions, actions);
        return this;
    };
    ...
    this.$get = ['$http', '$parse', '$routeRestful', function($http, $parse, $routeRestful) {
        ...
        function RestfulFactory(url, opts) {
            ...
            actions = angular.extend({}, providerActions, opts.actions);

            if (isArray(opts.only)) {
                var newActions = {};
                forEach(actions, function(value, key) {
                    if (opts.only.has(key)) newActions[key] = value;
                });
                actions = newActions;
            } else if (isArray(opts.except)) {
                var newActions = {};
                forEach(actions, function(value, key) {
                    if (!opts.except.has(key)) newActions[key] = value;
                });
                actions = newActions;
            }

            var route = new $routeRestful(url);
            ...
            forEach(actions, function(action, name) {
                ...
                Restful[name] = function(a1, a2, a3, a4, a5) {
                    var params = {},
                        ...
                        success = noop,
                        error = null,
                        baseURL = opts.baseURL || providerURL,
                        promise;
